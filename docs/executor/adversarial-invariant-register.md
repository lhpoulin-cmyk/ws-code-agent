...
| E10 | COVERED | `test_qwen_intent_records_exact_qwen_artifact_identity`; `test_devstral_intent_records_exact_devstral_artifact_identity`; `test_persisted_devstral_intent_cannot_resume_through_qwen_backend`; digest/profile mismatch tests; and `test_intent_survives_remote_success_disconnect_and_recovers_once` prove durable intent binds the exact model artifact/runtime profile and same-artifact recovery remains idempotent. |
