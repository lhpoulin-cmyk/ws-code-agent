...
class OllamaClient:
    ...
    def generate_audience(self, baseline: str, source: str, integrity: str, audience_contract: str, profile_contract_text: str, voice_contract: str, profile: AdapterProfile) -> GenerationResult:
        ...
        except (ValueError,json.JSONDecodeError) as exc: raise OllamaError(str(exc),raw_http,response_payload,"RESPONSE_SCHEMA_INVALID") from exc
        telemetry={k:response_payload[k] for k in ("total_duration","load_duration","prompt_eval_count","prompt_eval_duration","eval_count","eval_duration") if k in response_payload and isinstance(response_payload[k],(int,float))}; telemetry["wall_seconds"]=round(time.monotonic()-started,6)
        composed=audience_sha256(system); schema_hash=audience_sha256(serialized_json(audience_schema()))
        return GenerationResult(profile.model_identifier,digest,request_json,system,composed,started_at,completed_at,raw_http,response_payload,findings,proposal,exact_diff(baseline,proposal),sha256_text(raw_http),sha256_text(proposal),telemetry,"chat","/api/chat",profile.adapter_id,profile.profile_version,profile.request_serializer_version,("system","user"),{"audience":audience_sha256(audience_contract),"profile":audience_sha256(profile_contract_text),"voice":audience_sha256(voice_contract)},composed,AUDIENCE_VERSION,schema_hash,serialized_json(audience_schema()),"passed")
